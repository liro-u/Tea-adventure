...
public class CharacterControllerMotor : IMotor
{
    ...
    public virtual void ApplyForce(float tickDelta, bool? isGrounded = null)
    {
        ...
        Vector3 positionBefore = characterController.transform.position;
        characterController.Move(Velocity * tickDelta);

        // Sync velocity with actual displacement so wall/ceiling collisions cancel the blocked component.
        // We use tickDelta (not Time.deltaTime) so this is correct whether called from FixedUpdate or a custom tick loop.
        Vector3 actualVelocity = (characterController.transform.position - positionBefore) / tickDelta;
        if (Velocity != actualVelocity)
        {
