    ...
    public class SerializationTests
    {
        ...
        static void CompareUnityMaterial(Material lhs, Material rhs)
        {
            ...
            for (int i = 0; i < propertyCount; i++)
            {
                ...
                if (!lhs.HasProperty(prop) || !rhs.HasProperty(prop))
                    continue;
                ...
                switch (shader.GetPropertyType(i))
                {
                    case UnityEngine.Rendering.ShaderPropertyType.Color:
                        {
                            var l = lhs.GetColor(prop);
                            var r = rhs.GetColor(prop);
                            if (l != r)
                    ...
                    case UnityEngine.Rendering.ShaderPropertyType.Texture:
                        {
                            var lt = lhs.GetTexture(prop);
                            var rt = rhs.GetTexture(prop);

                            if (lt != rt)
                                sb.AppendLine($"{prop} texture {lt} != {rt}");

                            var lo = lhs.GetTextureOffset(prop);
                            var ro = rhs.GetTextureOffset(prop);
                            if (lo != ro)
                                sb.AppendLine($"{prop} offset {lo} != {ro}");

                            var ls = lhs.GetTextureScale(prop);
                            var rs = rhs.GetTextureScale(prop);
                            if (ls != rs)
                                sb.AppendLine($"{prop} scale {ls} != {rs}");
                        }
                        break;

                    default:
                        sb.AppendLine($"{prop} unsupported property type");
                        break;
                }
            }
